        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if opt.global_rank in [-1, 0]:
                ...
                if plots and ni < 3:
                    ...
                    Thread(target=plot_images, args=(imgs, targets, paths, f, None), daemon=True).start()
                    # if tb_writer:
...
if __name__ == '__main__':
    ...
    # Resume
    if cfg.resume:  # resume an interrupted run
        ckpt = cfg.resume if isinstance(cfg.resume, str) else get_latest_run()  # specified or most recent path
        assert os.path.isfile(ckpt), 'ERROR: --resume checkpoint does not exist'
        apriori = cfg.global_rank, cfg.local_rank
        with open(Path(ckpt).parent.parent / 'cfg.yaml') as f:
            cfg = argparse.Namespace(**yaml.load(f, Loader=yaml.SafeLoader))  # replace
        cfg.cfg, cfg.weights, cfg.resume, cfg.batch_size, cfg.global_rank, cfg.local_rank = '', ckpt, True, cfg.total_batch_size, *apriori  # reinstate
        logger.info('Resuming training from %s' % ckpt)
    else:
        # cfg.hyp = cfg.hyp or ('hyp.finetune.yaml' if cfg.weights else 'hyp.scratch.yaml')
    ...
    if not cfg.evolve:
        ...
        if cfg.global_rank in [-1, 0]:
            logger.info(f'Start Tensorboard with "tensorboard --logdir {cfg.project}", view at http://localhost:6006/')
            tb_writer = SummaryWriter(cfg.save_dir)  # Tensorboard
        train(hyp, cfg, device, tb_writer, wandb)

    # Evolve hyperparameters (cfgional)
    else:
        # Hyperparameter evolution metadata (mutation scale 0-1, lower_limit, upper_limit)
        meta = {'lr0': (1, 1e-5, 1e-1),  # initial learning rate (SGD=1E-2, Adam=1E-3)
                'lrf': (1, 0.01, 1.0),  # final OneCycleLR learning rate (lr0 * lrf)
                'momentum': (0.3, 0.6, 0.98),  # SGD momentum/Adam beta1
                'weight_decay': (1, 0.0, 0.001),  # cfgimizer weight decay
        ...
        assert cfg.local_rank == -1, 'DDP mode not implemented for --evolve'
        cfg.notest, cfg.nosave = True, True  # only test/save final epoch
        # ei = [isinstance(x, (int, float)) for x in hyp.values()]  # evolvable indices
        yaml_file = Path(cfg.save_dir) / 'hyp_evolved.yaml'  # save best result here
        if cfg.bucket:
            os.system('gsutil cp gs://%s/evolve.txt .' % cfg.bucket)  # download evolve.txt if exists
        ...
        for _ in range(300):  # generations to evolve
            ...
            # Train mutation
            results = train(hyp.copy(), cfg, device, wandb=wandb)

            # Write mutation results
            print_mutation(hyp.copy(), results, yaml_file, cfg.bucket)
